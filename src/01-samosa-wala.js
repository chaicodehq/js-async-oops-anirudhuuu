...
export function createSamosaCart(ownerName, location) {
  const cart = {
    ...
    sellItem(itemName, quantity) {
      if (
        !itemName ||
        !Object.prototype.hasOwnProperty.call(this.menu, itemName) ||
      ...
      const sale = { item: itemName, quantity, total };
    ...
    getPopularItem() {
      ...
      for (const sale of this.sales) {
        counts.set(sale.item, (counts.get(sale.item) || 0) + sale.quantity);
      }
      let popular = null;
      let maxQty = -1;
      for (const [item, qty] of counts.entries()) {
        if (qty > maxQty) {
          maxQty = qty;
          popular = item;
        }
