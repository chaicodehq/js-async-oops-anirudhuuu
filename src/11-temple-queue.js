...
export class TempleQueue {
  ...
  static merge(queue1, queue2) {
    ...
    const merged = new TempleQueue(mergedName, mergedCapacity);
    
    const q1Array = queue1.toArray();
    const q2Array = queue2.toArray();
    
    for (const devotee of q1Array) {
      merged.#devotees.push({ ...devotee });
    }
    for (const devotee of q2Array) {
      merged.#devotees.push({ ...devotee });
    }
