...
LassiStand.prototype.addFlavor = function (flavor, price) {
  if (!flavor || typeof price !== "number" || price <= 0) {
    return -1;
  }
  const exists = this.menu.some((f) => f.flavor === flavor);
  if (exists) return -1;
...
LassiStand.prototype.takeOrder = function (customerName, flavor, quantity) {
  ...
  const flavorObj = this.menu.find((f) => f.flavor === flavor);
  if (!flavorObj) return -1;

  const id = this._nextOrderId++;
  const total = flavorObj.price * quantity;
  const order = {
    id,
